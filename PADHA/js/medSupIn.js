...
supAddItem.onclick = function () {
    let newItem = {
        supDivision: supDivision.value.toLowerCase(),
    ...
    )
    {
        ...
        } else
        {
            ...
            mood = 'add';
            supAddItem.innerHTML = '<i class="fa-solid fa-plus"></i>add item to Chart';
        }
        clearData();
    }
...
function showData() {
    ...
    for (let i = 0; i < dataItem.length; i++)
    {
        ...
        <tr>
            ...
            <td class="text-nowrap">${dataItem[i].supDivision}</td>
            ...
            <td class="text-nowrap"><button onclick=editData(${i}) class="btn btn-master" id="supEdit">Edit</button></td>
            <td class="text-nowrap"><button onclick=deleteData(${i}) class="btn btn-red" id="supDelete">Delete</button></td>
        </tr> 
        `;
    }
    ...
    if (dataItem.length > 0)
    {
        supDeleteAll.innerHTML =
            `
        <button onclick="deleteAll()" class="btn btn-red"><i class="fa-solid fa-trash-can"></i>Delete All Items in Chart ( ${dataItem.length} items )</button>
        `;
        ...
        <div class="mb-4 row g-1 justify-content-end mt-4">
            <button class="btn btn-padha col-md-4" type="submit" id="submitStaticsToServer">
                <i class="fa-solid fa-arrow-right-to-bracket"></i>Confirm Input</button>
        </div>
        `;
    }
    else
...
function editData(i) {
    supDivision.value = dataItem[i].supDivision;
    ...
    supAddItem.innerHTML = '<i class="fa-solid fa-pen-to-square"></i>Edit item Data';
    mood = 'edit';
    temporaryMood = i;
    scroll({
        top: 0,
...
function supDivisionValid() {
    let supDivValidation = document.getElementById('supDivisionValid');
    if (!supDivision.checkValidity())
    {
        supDivValidation.innerHTML = `<i class="fa-solid fa-circle-exclamation"></i>Choose Division`;
        supDivValidation.style.color = "var(--red-color)";
        supDivValidation.style.marginTop = "5px";
        supDivision.classList.remove("valid");
        supDivision.classList.add("inValid");
    } else
    {
        supDivValidation.innerHTML = `<i class="fa-solid fa-check"></i>`;
        supDivValidation.style.color = "var(--green-color)";
        supDivValidation.style.marginTop = "5px";
        supDivision.classList.remove("inValid");
        supDivision.classList.add("valid");
    }
    if (supDivision.value === "")
    {
        supDivValidation.innerHTML = `<i class="fa-solid fa-circle-exclamation"></i>Choose Division`;
        supDivValidation.style.color = "var(--red-color)";
        supDivValidation.style.marginTop = "5px";
...
function supDivisionClear() {
    let supDivValid = document.getElementById('supDivisionValid');
    supDivision.value = "";
    supDivValid.innerHTML = "";
    supDivValid.style.color = "var(--darkest-color)";
    supDivValid.style.marginTop = "0";
    supDivision.classList.remove("valid");
    supDivision.classList.remove("inValid");
};
